var demo = (function () {
  ...
  var require$$0 = getCjsExportFromNamespace(entities$1); // HTML5 entities map: { name -> utf16string }
  //
    ...
    for (i = inlineTokens.length - 1; i >= 0; i--) {
      ...
      if (token.type === 'text' && !inside_autolink) {
        if (RARE_RE.test(token.content)) {
          ...
          // but ?..... & !..... -> ?.. & !..
          .replace(/([?!])\.{4,}/g, '$1..').replace(/\.{3,}/g, '…').replace(/…\.+/g, '…') // also remove superfluous periods after an ellipsis
          ...
          // ==>
          // -->
          .replace(/(^|[^<=-])([<]?(?:==|--)[>]?)(?=[^>=-]|$)/mg, replace_arrow) // and do it once more to catch input which overlaps in the detection
          // regex, e.g. `<--x-->` where the `-->` will not be detected by
          // the above `replace()` due to overlap at `x` in the rwuired match:
          .replace(/(^|[^<=-])([<]?(?:==|--)[>]?)(?=[^>=-]|$)/mg, replace_arrow) // em-dash
  ...
  var _rules = [['normalize', normalize], ['block', block$1], ['inline', inline], ['replacements', replacements], ['smartquotes', smartquotes]];
  /**
   * new Core()
   **/
  ...
  var parser_core = Core; // GFM table, non-standard
  ...
  var table = function table(state, startLine, endLine, silent) {
    var ch, lineText, lineTextReplaced, pos, i, nextLine, columns, columnCount, token, aligns, t, tableLines, tbodyLines, positions, len, columnVIndex; // should have at least two lines
    ...
    positions = [];
    columns = escapedSplit(lineTextReplaced, positions); // header row will define an amount of columns in the entire table,
    ...
    for (i = 0; i < columns.length; i++) {
      ...
      token.position = columnVIndex; // Last column?
  ...
  var list = function list(state, startLine, endLine, silent) {
    ...
    while (nextLine < endLine) {
      ...
      token.position = contentStart; // change current state, then restore it after parser subcall
  ...
  var getLineOffset = utils.getLineOffset; // Insert each marker as a separate text token, and add it to delimiter list
  //
  ...
  var _rules$2 = [['linkify', linkify.tokenize], ['text', text], ['newline', newline], ['escape', _escape], ['backticks', backticks], ['strikethrough', strikethrough.tokenize], ['emphasis', emphasis.tokenize], ['link', link], ['image', image$1], ['autolink', autolink], ['html_inline', html_inline], ['entity', entity]];
  ...
  var decode$2 = function decode$2(input) {
    // Don't use UCS-2.
    var output = [];
  ...
  var encode$2 = function encode$2(input) {
  ...
  function renderPermalink(slug, opts, state, idx) {
    ...
    var linkTokens = [Object.assign(new state.Token('link_open', 'a', 1), {
      attrs: [['class', opts.permalinkClass], ['href', opts.permalinkHref(slug, state)]].concat(Object.entries(opts.permalinkAttrs(slug, state)))
  ...
  var anchor = function anchor(md, opts) {
    ...
    md.core.ruler.push('anchor', function (state) {
      ...
      var htoks = tokens.filter(function (token) {
        return token.type === 'heading_open';
      });
      htoks.forEach(function (token) {
      ...
      htoks.filter(function (token) {
        return isLevelSelected(Number(token.tag.substr(1)));
      }).forEach(function (token) {
        // Aggregate the next token children text.
        var idx = tokens.indexOf(token);
        var title = tokens[idx + 1].children.filter(function (child_token) {
          return child_token.type === 'text' || child_token.type === 'code_inline';
  ...
  /*! markdown-it-toc-done-right 4.1.0-3 https://github.com//GerHobbelt/markdown-it-toc-done-right @license MIT */
    ...
    function headings2ast(tokens) {
      ...
      for (var i = 0, iK = tokens.length; i < iK; i++) {
        ...
        if (token.type === 'heading_open') {
          var key = tokens[i + 1].children.filter(function (token) {
