...
var demo = (function () {
    ...
    for (i = inlineTokens.length - 1; i >= 0; i--) {
      ...
      if (token.type === 'text' && !inside_autolink) {
        if (RARE_RE.test(token.content)) {
          token.content = token.content.replace(/\+-/g, '±').replace(/([?!])\.{4,}/g, '$1..').replace(/\.{3,}/g, '…').replace(/…\.+/g, '…').replace(/([?!]){4,}/g, '$1$1$1').replace(/,{2,}/g, ',').replace(/(^|[^<=-])([<]?(?:==|--)[>]?)(?=[^>=-]|$)/mg, replace_arrow).replace(/(^|[^<=-])([<]?(?:==|--)[>]?)(?=[^>=-]|$)/mg, replace_arrow).replace(/(^|[^-])---(?=[^-]|$)/mg, "$1\u2014").replace(/(^|\s)--(?=\s|$)/mg, "$1\u2013").replace(/(^|[^-\s])--(?=[^-\s]|$)/mg, "$1\u2013");
        }
      }
  ...
  var table = function table(state, startLine, endLine, silent) {
    var ch, lineText, lineTextReplaced, pos, i, nextLine, columns, columnCount, token, aligns, t, tableLines, tbodyLines, positions, len, columnVIndex;
  ...
  var demo = {
    ...
    uslug: uslug$1,
    markdownItTocDoneRight: tocPlugin
  };
